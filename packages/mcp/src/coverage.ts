...
export function mcpSynthesisCoverage(
  ...
  for (const binding of Object.values(iface.bindings ?? {})) {
    if (binding.selector) represented.set(binding.selector, { operationKey: binding.operation, bindingSelector: binding.selector });
  }
  ...
  disc.tools.forEach((entity, index) => {
    ...
    else if (count > 1) add("tools", entity.name, index, count, { status: "excluded", reasonCode: "mcp.ambiguous_identity", rule: "MCP-P-02", message: "more than one listed tool has this selector identity" });
  ...
  disc.resources.forEach((entity, index) => {
    ...
    else if (count > 1) add("resources", entity.uri, index, count, { status: "excluded", reasonCode: "mcp.ambiguous_identity", rule: "MCP-P-02", message: "more than one listed resource has this selector identity" });
  ...
  disc.resourceTemplates.forEach((entity, index) => {
    ...
    else if (count > 1) add("resourceTemplates", entity.uriTemplate, index, count, { status: "excluded", reasonCode: "mcp.ambiguous_identity", rule: "MCP-P-02", message: "more than one listed resource template has this selector identity" });
    else {
      try {
  ...
  disc.prompts.forEach((entity, index) => {
    ...
    else if (count > 1) add("prompts", entity.name, index, count, { status: "excluded", reasonCode: "mcp.ambiguous_identity", rule: "MCP-P-02", message: "more than one listed prompt has this selector identity" });
