...
var parseSignature = require('./signature.cjs');
var numeric = require('./number-lexeme.cjs');
...
const parser = (() => {
    ...
    var tokenizer = function (path) {
        ...
        var scanRegex = function () {
            ...
            // Delimiters inside character classes or escapes are pattern text.
            var start = position;
            var inClass = false;
            var pattern;
            var flags;
            ...
            var isClosingSlash = function (position) {
                // The scanning loop consumes escapes in pairs before calling
                // this predicate; an escaped slash cannot reach this point.
                return path.charAt(position) === '/' && !inClass;
            };

            while (position < length) {
                var currentChar = path.charAt(position);
                if (currentChar === '\\') { position += 2; continue; }
                ...
                if (currentChar === '[') { inClass = true; }
                if (currentChar === ']') { inClass = false; }
        ...
        var next = function (prefix) {
            ...
            if (match !== null) {
                var num = numeric.fromText(match[0]);
                // fromText either admits an exact number or throws. The old
                // native-infinity branch is unreachable in this value model.
    ...
    var parser = function (source, recover) {
        ...
        var expression = function (rbp) {
            var left;
            var t = node;
            // Prefix operators that consume an expression need a prefix token
            // next; otherwise a leading regex is misread as division.
            advance(null, ['(', '[', '{', '-', '|'].indexOf(t.id) === -1);
            left = t.nud();
            while (rbp < node.lbp) {
                t = node;
                advance();
                ...
                case 'unary':
                    ...
                    if (expr.value === '[') {
                        ...
                        if (expr.value === '-' && result.expression.type === 'number') {
                            ...
                            result.value = numeric.fromText(numeric.text(result.value)[0] === '-' ? numeric.text(result.value).slice(1) : '-' + numeric.text(result.value));
                        } else {
