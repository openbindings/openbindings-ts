...
describe("exact schema predicates and diagnostics", () => {
  for (const [schema, input, valid] of [
    ...
    expect(compileEmbeddedSchema(parseJSON(schema)).validate(parseJSON(input)).valid).toBe(valid);
  });
  for (const schema of ['{"const":1}', '{"type":"string"}', '{"enum":[1]}',
    ...
    const result = compileEmbeddedSchema(parseJSON(schema)).validate(parseJSON('9007199254740993'));
