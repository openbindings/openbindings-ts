 ...
 * resolution, override, pinning, default policy). The SDK has no
 * planOperation diagnostics probe (the Go harness's observation point), so
 * the selected key is observed via the invocation-site carriage the invoke
 * path stamps on the binding-layer args (`args.site.bindingKey`) — public
 * contract surface, no test seam added.
 */
