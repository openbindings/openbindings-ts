...
const corpusEntries = [
  {
    ...
    wanted: new Set([
      ...
      "OAPI20-PS-128", "OAPI20-PS-129", "OAPI20-PS-130", "OAPI20-PS-131", "OAPI20-PS-132", "OAPI20-PS-133", "OAPI20-PS-134", "OAPI20-PS-135", "OAPI20-PS-136",
    ]),
  },
...
describe("portable OpenAPI processor scenarios", () => {
  ...
  afterAll(() => {
    expect(corpora.map(({ corpus, wanted }) => wanted?.size ?? corpus.scenarios.length)).toEqual([136, 110, 138, 212]);
    expect(executedByCorpus).toEqual([136, 110, 138, 212]);
    expect(executed).toBe(596);
  });
});
