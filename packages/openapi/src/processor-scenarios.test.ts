...
describe("portable OpenAPI processor scenarios", () => {
  ...
  afterAll(() => {
    expect(corpora.map(({ corpus, wanted }) => wanted?.size ?? corpus.scenarios.length)).toEqual([135, 116, 141, 213]);
    expect(executedByCorpus).toEqual([135, 116, 141, 213]);
    expect(executed).toBe(605);
  });
});
